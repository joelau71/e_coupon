$(function() {
    ...
    uploadImageCropper = {
        init: function() {
            ...
            this.$body.on("change", ".upload-image-cropper", function() {
                var $this = $(this);
            ...
            $(document).on("drop", ".uic-label-wrapper", function(e) {
                ...
                var $upload = $wrapper.find(".upload-image-cropper");
                ...
                document.querySelector(".upload-image-cropper").files =
                ...
                if (
                    ...
                    alert("Only Accept jpg, png, webp file format!");
                    return false;
                }
                ...
                self.resizeImage(
                    ...
                    $("#upload_image_preview").removeAttr("src");
                    $(".image-cropper-wrapper").hide();
                    if (typeof self.uploadImageCompleteHook === "function")
                        self.uploadImageCompleteHook(name, res);
                });
            });
        ...
        update: function() {
            var self = this;
            $(".upload-image-cropper").each(function() {
                var $this = $(this);
                if ($this.hasClass("is-upload-image-cropper")) return true;
                self.wrapper($this);
            });
        },
        ...
        wrapper: function($element) {
            ...
            $element.addClass("is-upload-image-cropper");
            ...
            if ($element.attr("data-uic-display-width") == undefined) {
                ...
                $label.wrap(
                    `<div class="upload-image-cropper-wrapper ${hasData}"></div>`
                );
            } else {
                if (path) {
                    $label.append(
                        `<img class="uic-preview-image" src="${path}" />`
                    );
                } else {
                    $label.append(
                ...
                $label.wrap(
                    `<div class="upload-image-cropper-wrapper ${hasData}" style="max-width: ${dWidth}px"></div>`
                );
            }
            ...
            $wrapper.append(
                `<input type="hidden" class="uic-base64" name="uic_base64_${name}" id="uic_base64_${name}" />`
            );
        ...
        destroy: function() {
            $(".image-cropper-wrapper").remove();
        }
    };
    uploadImageCropper.init();
});
